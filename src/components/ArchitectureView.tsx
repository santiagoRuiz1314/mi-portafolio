    ...
    <div className={cn('space-y-6', className)}>
      ...
      <div className="bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-700 rounded-xl p-6 border border-gray-200 dark:border-gray-600">
        <div className="flex items-start space-x-4">
          <div className={`p-3 rounded-lg bg-${typeInfo.color}-100 dark:bg-${typeInfo.color}-800`}>
      ...
      return (
        <div key={layer.id} className="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
          <button
            ...
            className="w-full p-4 text-left bg-gray-50 dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors flex items-center justify-between"
          >
            <div className="flex items-center space-x-3">
              <span className="text-2xl">{layer.icon}</span>
              <div>
                <h4 className="font-semibold text-gray-900 dark:text-white">{layer.name}</h4>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {layer.technologies.map((tech, techIndex) => (
                      <span
                        key={techIndex}
                        className="px-2 py-1 text-xs bg-primary-100 dark:bg-primary-800 text-primary-800 dark:text-primary-200 rounded-full"
                      >
                        {tech}
                      </span>
                    ))}
                  ...
                  <ul className="space-y-1">
                    {layer.responsibilities.map((responsibility, respIndex) => (
                      <li key={respIndex} className="flex items-start space-x-2">
                        ...
                        <span className="text-xs text-gray-600 dark:text-gray-400">{responsibility}</span>
                      </li>
                    ))}
                  </ul>
                </div>
...
const DataFlowView: React.FC<{ dataFlow: string[] }> = ({ dataFlow }) => (
  <div className="space-y-4">
    <h4 className="text-lg font-semibold text-gray-900 dark:text-white flex items-center">
      <ArrowDown className="w-5 h-5 mr-2 text-primary-500" />
      Data Flow Process
    </h4>
    <div className="space-y-3">
    ...
    <div className="space-y-4">
      ...
      <ul className="space-y-3">
        {considerations.map((consideration, index) => (
          <li key={index} className="flex items-start space-x-3 p-3 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg">
